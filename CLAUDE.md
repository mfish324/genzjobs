...
- **`prisma.config.ts` placeholder pattern**: Prisma 7's `env()` helper resolves strictly at config-load. Vercel's `npm install` runs `postinstall: prisma generate` without `DATABASE_URL` set, which blows up `env()`. Use `process.env.DATABASE_URL ?? "postgresql://placeholder@localhost:5432/placeholder"` instead. At runtime the Neon adapter reads `DATABASE_URL` directly via `PrismaNeon({ connectionString: ... })`, so the placeholder is only used for offline CLI work.
...
- **Scraper tz-aware datetimes**: Scrapers that parse `posted_at` via `fromisoformat(...replace("Z","+00:00"))` produce offset-aware datetimes. The `postedAt` column is `TIMESTAMP` (no TZ), so asyncpg rejects them. `database.save_jobs()` normalizes via `_to_naive_utc()` before binding — don't bypass it.
- **Railway scraper URL**: `https://genzjobs-production.up.railway.app`
- **Railway `watchPatterns`**: `railway.toml` scopes auto-deploy to `scraper/**`. Commits touching only top-level files (e.g., `prisma.config.ts`, root `package.json`, `scripts/`) won't trigger a Railway redeploy — you must `railway service redeploy --yes` manually if a re-run is needed.
- **Railway requires `PYTHONUNBUFFERED=1`**: without it, Python buffers stdout/stderr, so logs don't appear in Railway in real time and a hung container looks identical to a healthy one. Set it as a service env var (already set in production). Long-term, should be added to the Dockerfile as a permanent ENV directive.
- **Railway CLI logs on Windows**: `railway logs --deployment ...` returns 0 lines on Windows even when logs exist in the dashboard. Use the dashboard for runtime logs — the CLI works fine for build logs and `service status`. Cause appears to be a known CLI quirk on Windows; not worth fixing locally.
